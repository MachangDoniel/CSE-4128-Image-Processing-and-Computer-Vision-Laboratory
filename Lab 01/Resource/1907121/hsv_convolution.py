...
def convolution_hsv(image, kernel, kernel_center=(-1,-1)):
    ...
    hue_conv = convolution(image=hue, kernel=kernel, kernel_center=kernel_center)
    hue_nor = normalize(hue_conv)
    
    sat_conv = convolution(image=sat, kernel=kernel, kernel_center=kernel_center)
    sat_nor = normalize(sat_conv)
    
    val_conv = convolution(image=val, kernel=kernel, kernel_center=kernel_center)
...
# convolution_hsv(image=image, kernel=kernel, kernel_center=(-1,-1))
